...
function getandsavefbreviews(pageid,pagetoken,pagename,reviewarray,totalinserted,numtodownload,aftercode){	
	...
	FB.api(pageid+'/ratings', {
		...
		if (typeof(response.data) == 'undefined') {
			alert("Error 101: can not get reviews from Facebook. "+response);
		}
				...
				//console.log(response);
					...
					jQuery("#page_list" ).html("");
						...
						for (i = 0; i < fbpagearray.length; i++) { 
							tablerows = tablerows + '<tr id="" class=""><td><button onclick=\'getfbreviewsfunction("' + fbpagearray[i].id + '", "' + fbpagearray[i].access_token + '", "' + fbpagearray[i].name + '")\' id="getreviews_' + fbpagearray[i].id + '" type="button" class="btn_green">Retrieve Reviews</button></td> \
